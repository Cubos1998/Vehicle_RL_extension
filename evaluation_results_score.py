...
def main():
    ...
    parser.add_argument('--folder', type=str,
                        default='/home/cubides/MA/DonkeyCar/ROS_DonkeyCar/src/results_reality/',
    ...
    parser.add_argument('--safe_thresholds', type=str, default='1.5,2.5,4.0',
                        help="Comma-separated list of three safe CTE thresholds.")
    parser.add_argument('--ideal_angle', type=float, default=360.0,
                        help="Ideal cumulative turning angle for a perfect score (default 360).")
    ...
    for run_key, files in runs.items():
        try:
            ...
            # Compute average safe percentage across the provided safe thresholds.
            avg_safe = sum(safe_percents[st] for st in safe_thresholds) / len(safe_thresholds)
            # Compute the performance Score.
            # Ideal performance: survival_turn = ideal_angle and avg_safe = 100 gives Score = 100.
            score = (surv_turn / ideal_angle) * avg_safe
    ...
    # --- Group results by the subfolder (Group) using only the mean (no std) ---
    agg_dict = {}
    for col in df.columns:
        if col in ["Run", "Group"]:
            continue
        # For MaxCTE use max, for others use mean.
        if col == "MaxCTE":
            agg_dict[col] = "max"
        else:
            agg_dict[col] = "mean"
